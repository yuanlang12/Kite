...
> **⚠️ Important: Only one Kite process per bot.** If you start kite in multiple project directories, all IM messages will be received by all processes, causing duplicate replies. Instead, run a single kite process and use [multi-project routing](#multi-project-routing) to manage different projects.
...
| `/status` | Show current settings and bound project |
| `/approve <code>` | Approve a new user's pairing code |
| `/addproject <name> <path>` | Register a project |
| `/rmproject <name>` | Remove a registered project |
| `/bind <name>` | Bind this chat to a project |
| `/unbind` | Remove binding (revert to default) |
| `/projects` | List all projects and bindings |
...
A single Kite process can manage multiple projects. Different IM chats/groups can be bound to different projects.

**Why only one Kite?** Each IM bot can only be connected by one process at a time. Running multiple kite instances with the same bot token causes duplicate replies.

### Usage

```bash
# Start kite in your main project (only once)
cd ~/code/my-app
kite
```
...
```
/addproject blog ~/code/blog        # Register another project
/bind blog                          # Bind this chat to it
/unbind                             # Revert to default project
```

On first message, if multiple registered projects exist and the chat is not bound, Kite will prompt you to choose:
